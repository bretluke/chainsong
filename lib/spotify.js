...
// Words that mark a non-canonical cut. If the user didn't ask for one of
// these in their own title, we drop candidates whose Spotify name mentions
// them, then rank what's left by popularity — this keeps karaoke and
// instrumental versions from outranking the real hit (see "The Monster"
// instrumental beating out the feat.-Rihanna original).
...
async function searchTrack(title, artist) {
  ...
  const url = `${API_BASE}/search?type=track&limit=10&q=${encodeURIComponent(q)}`;
  try {
    ...
    // If our filter would leave nothing (song only exists as an alt cut),
    // fall back to the raw list so we still return something.
    const pool = filtered.length ? filtered : items;
    const t = pool.slice().sort((a, b) => (b.popularity ?? 0) - (a.popularity ?? 0))[0];
...
async function exchangeCodeForTokens(code, redirectUri) {
  ...
  const data = await res.json();
  console.log('SPOTIFY_GRANTED_SCOPES:', data.scope);   // <-- add this line
  return {
...
async function createPlaylistWithTracks(accessToken, userId, name, description, trackUris) {
  console.log('CREATE_PLAYLIST_ATTEMPT', { userId, name, urisCount: trackUris.length });
  ...
  if (!createRes.ok) {
    ...
    const wwwAuth = createRes.headers.get('www-authenticate');
    throw new Error(`create playlist failed: ${createRes.status} body=${bodyText} wwwauth=${wwwAuth}`);
  }
  ...
  if (trackUris.length) {
    ...
    if (!addRes.ok) {
      ...
      throw new Error(`add tracks failed: ${addRes.status} ${bodyText}`);
    }
  }
